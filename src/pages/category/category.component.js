...
const Category = ({ per }) => {
    ...
    if (data) {
        ...
        cateList = [...data.categories];
        dataProducts = dataProducts.filter((pro, index) => {
            if (category === 'all') {
                return pro && index < per;
            }
    ...
    return (
        <>
            <div className="flex ml-2 text-lg font-bold">
                <div>
                    <span
                        onClick={() => {
                            setCategory('all')
                        }}
                        className={`mx-3 my-2 cursor-pointer ${category === 'all' ? 'underline' : ''}`}>
                        All
                    </span>
                </div>
                <div>
                    {
                        cateList?.map((cate, index) => (
                            <span
                                ...
                                className={`mx-3 my-2 cursor-pointer ${category === cate ? 'underline' : ''}`}
                            >
                                {cate}
                            </span>
                        ))
                        ...
                        <div key={pro.id} className="w-full h-[23rem] p-4 bg-gray-50 rounded-3xl">
                            ...
                            <div className='flex justify-between'>
                                ...
                                <button
                                    ...
                                    className="flex items-center justify-center w-3/5 px-2 py-2 mt-4 font-medium bg-gray-300 rounded-xl hover:bg-gray-400 focus:outline-none focus:bg-gray-400">
